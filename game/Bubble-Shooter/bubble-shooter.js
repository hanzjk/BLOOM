...
window.onload = function() {
    ...
    var level = {
        ...
        rows: 11, // Number of tile rows
        tilewidth: 40, // Visual width of a tile
        tileheight: 40, // Visual height of a tile
        rowheight: 34, // Height of a row
        radius: 19, // Bubble collision radius
    ...
    // Game states
    var gamestates = { init: 0, ready: 1, shootbubble: 2, removecluster: 3, gameover: 4, victory: 5 };
    ...
    function checkGameVictory() {
        ...
        for (var i = 0; i < level.columns; i++) {
            // Check if there are bubbles in the top row
            if (level.tiles[0][i].type != -1) {
                // Game Won
                nextBubble();
                setGameState(gamestates.victory);
                return true;
    ...
    function findCluster(tx, ty, matchtype, reset, skipremoved) {
        ...
        // Get the target tile. Tile coordinates must be valid.
    ...
    function render() {
        ...
        context.font = "18px Grandstander, cursive";
        var scorex = level.x + level.width - 150;
        var scorey = level.y + level.height + level.tileheight - yoffset - 20;
        drawCenterText("Score:", scorex, scorey, 150);
        context.font = "24px Grandstander, cursive";
        drawCenterText(score, scorex, scorey + 30, 150);

        /* //draw help
         var c = document.getElementById("viewport");
         var ctx = c.getContext("2d");
         var soundx = level.x + level.width - 200;
         var soundy = level.y + level.height + level.tileheight - yoffset - 30;
         var img = document.getElementById("help");
         ctx.drawImage(img, soundx, soundy, 60, 70); */
        // Render cluster
        if (showcluster) {
        ...
        if (gamestate == gamestates.gameover) {
            ...
            context.fillStyle = "#cc0044";
            context.font = "40px Grandstander, cursive";
            drawCenterText("Game Over!", level.x, level.y + level.height / 4 + 10, level.width);
            drawCenterText("Your Score:", level.x, level.y + level.height / 4 + 60, level.width);
            drawCenterText(score, level.x, level.y + level.height / 4 + 100, level.width);
            drawCenterText("Click to Play Again", level.x, level.y + level.height / 4 + 170, level.width);
        }
        ...
        if (gamestate == gamestates.victory) {
            ...
            context.fillStyle = "#cc0044";
            context.font = "40px Grandstander, cursive";
            drawCenterText("Congratulations! You Won!", level.x, level.y + level.height / 4 + 10, level.width);
            drawCenterText("Your Score:", level.x, level.y + level.height / 4 + 60, level.width);
            drawCenterText(score, level.x, level.y + level.height / 4 + 100, level.width);
            drawCenterText("Click to Play Again", level.x, level.y + level.height / 4 + 170, level.width);
        }
    ...
    function drawFrame() {
        ...
        // context.fillText("Bubble Shooter", 170, 35);

        // Display fps
        // context.fillStyle = "#0000ff";
        //context.font = "20px Grandstander, cursive";
    ...
    function renderPlayer() {
        ...
        context.beginPath();
        context.arc(centerx, centery, level.radius + 6, 0, 2 * Math.PI, false);
        context.fill();
        context.lineWidth = 2;
